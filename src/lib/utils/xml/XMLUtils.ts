...
export const readAbsXMLFile = <T>(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (rawFile && typeof rawFile === "string") {
      const XmlObject = await parseXMLObjectAsync(rawFile);
...
export const getOrCreate = <T>(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!rawFile) {
      ...
      return await readAbsXMLFile<T>(absolutePath);
    }
  });
};

export const parseXMLObjectAsync = (xmlRawString: string): Promise<any> => {
  const sanitizedXml = safeString(xmlRawString);
  const parseString = promisify(new XML2JS.Parser().parseString);
  return parseString(sanitizedXml);
};

export const exportNode = (node: any) => {
...
export const unicodeCharToHexCodePoint = function(char: string) {
